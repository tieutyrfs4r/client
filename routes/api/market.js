...
router.get('/', async (req, res, next) => {
    try {
        ...
        const filteredCurrencyData = currencyData.filter(item => item.cryptocurrency_name !== 'USDT').map(item => {
            const {cryptocurrency_name,lastPrice,priceChange,priceChangePercent,volume24h,img_url} = item
            return {cryptocurrency_name,lastPrice,priceChange,priceChangePercent,volume24h,img_url};
        });
